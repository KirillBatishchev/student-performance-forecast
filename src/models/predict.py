...
def load_model():
    """Загрузить модель"""
    try:
        model = mlflow.pytorch.load_model("models:/SimpleDKT/latest", map_location="cpu")
        model.eval()
        return model
...
def predict(user_ids):
    ...
    means, stds = params["normalization"]["means"], params["normalization"]["stds"]
    ...
    for user_id in user_ids:
        try:
            ...
            results.append({"user_id": user_id, "prediction": round(pred, 4), "will_succeed": pred > 0.5})
            print(f"{user_id}: {pred:.3f} - {'успех' if pred > 0.5 else 'риск'}")
    ...
    st.save_json(output, f"predictions/{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
    
    return results

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--users", nargs="+", required=True, help="Список user_id")
